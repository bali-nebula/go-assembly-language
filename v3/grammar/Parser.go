...
func (v *parser_) parseAction() (
	...
	// Attempt to parse a single Skip Action.
	var skip ast.SkipLike
	skip, token, ok = v.parseSkip()
	if ok {
		// Found a single Skip Action.
		action = ast.ActionClass().Action(skip)
		return
	}
...
func (v *parser_) parseSkip() (
	...
	// Attempt to parse a single "SKIP" literal.
	var delimiter string
	delimiter, token, ok = v.parseDelimiter("SKIP")
	if !ok {
		if uti.IsDefined(tokens) {
			// This is not a single Skip rule.
			v.putBack(tokens)
			return
		} else {
			// Found a syntax error.
			var message = v.formatError("$Skip", token)
			panic(message)
		}
	}
	if uti.IsDefined(tokens) {
	...
	// Found a single Skip rule.
	ok = true
	v.remove(tokens)
	skip = ast.SkipClass().Skip(delimiter)
	return
}
